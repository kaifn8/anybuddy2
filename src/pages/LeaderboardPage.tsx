...
function generateLeaderboard(userXp: number, userName: string, userStreak: number) {
  ...
  const realUser = { userId: 'me', name: userName || 'You', xp: userXp, weeklyXp: userXp, level: getLevelForXP(userXp * 8).level, streak: userStreak };
...
export default function LeaderboardPage() {
  ...
  const board = useMemo(() => generateLeaderboard(weeklyXp, user?.firstName || 'You', streak.count), [weeklyXp, user?.firstName, streak.count]);
      ...
      <div className="mobile-container min-h-screen bg-background pb-28">
        ...
        <div className="px-5 pt-5 space-y-3">
          ...
          <div className="liquid-glass-heavy p-4 text-center">
            <p className="text-[11px] text-muted-foreground uppercase tracking-wider font-semibold mb-1">📍 Mumbai · Weekly XP</p>
              ...
              return (
                <div key={entry.userId} className={cn('flex-1 flex flex-col items-center gap-2', idx === 1 && 'scale-105')}>
                  <div className="relative">
                    <GradientAvatar name={entry.name} size={idx === 1 ? 50 : 40}
                  ...
                  <div className={cn(
                    ...
                    heights[podiumIdx],
                    idx === 1 ? 'bg-accent/10 border border-accent/20' : 'liquid-glass'
                  )}>
                    <p className="text-[11px] font-bold text-foreground truncate px-2 text-center">{entry.name.split(' ')[0]}</p>
                ...
                <div key={entry.userId}
                  ...
                  <div className="w-7 text-center shrink-0">
                    {rank <= 3
                      ...
                      : <span className={cn('text-[12px] font-bold tabular-nums', isMe ? 'text-primary' : 'text-muted-foreground/50')}>#{rank}</span>
                    }
                  </div>
