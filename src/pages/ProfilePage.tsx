            ...
            <div className="w-full h-2 rounded-full overflow-hidden" style={{ background: 'rgba(0,0,0,0.04)' }}>
              <div 
                ...
                style={{ 
                  ...
                  background: 'linear-gradient(90deg, hsl(213 94% 55%), hsl(250 85% 60%))',
                }}
              />
            </div>
      ...
      <div className="mobile-container min-h-screen bg-ambient pb-28">
        ...
        <div className="px-4 pt-2 space-y-3">
          ...
          >
            ...
            <Share2 size={15} className="text-primary/60 shrink-0" />
          </button>
          ...
          <div className="flex gap-1 p-1" style={{
            ...
            {(['overview', 'activity'] as const).map((tab) => (
              <button
                ...
                className={cn(
                  'flex-1 py-2 text-xs font-semibold transition-all tap-scale capitalize',
                  activeTab === tab
                ...
                <Section title="Interests">
                  <div className="flex flex-wrap gap-2">
                    {user.interests.map((interest) => (
                      <div key={interest} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium" style={{
                        borderRadius: '12px',
                        background: 'rgba(255,255,255,0.65)',
                ...
                <Section title="Badges">
                  <div className="flex flex-wrap gap-2">
                    {user.badges.map((badge) => (
                      <div key={badge} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold" style={{
                        borderRadius: '12px',
                        background: 'linear-gradient(135deg, rgba(139,92,246,0.06), rgba(59,130,246,0.04))',
                      }}>
...
function StatTile({ icon, label, value }: { icon: string; label: string; value: string }) {
  return (
    <div className="flex items-center gap-2.5 p-3" style={{
      borderRadius: '16px',
      background: 'rgba(255,255,255,0.65)',
      boxShadow: '0 1px 4px rgba(0,0,0,0.03)',
    }}>
...
function RequestRow({ req, onClick, subtitle }: { req: Request; onClick: () => void; subtitle?: string }) {
  return (
    <button onClick={onClick}
      className="w-full flex items-center gap-3 p-3 text-left tap-scale transition-all group"
      style={{
        borderRadius: '16px',
        background: 'rgba(255,255,255,0.65)',
        boxShadow: '0 1px 4px rgba(0,0,0,0.03)',
      }}>
      <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-lg" style={{
        background: 'rgba(0,0,0,0.03)',
      }}>
