...
export default function CreateRequestPage() {
  ...
  useEffect(() => {
    if (costRef.current) {
      gsap.fromTo(costRef.current, { scale: 1.2 }, { scale: 1, duration: 0.3, ease: 'back.out(2)' });
    }
  }, [creditCost]);
  ...
  const handleSubmit = () => {
    ...
    if (urgency === 'now') {
      expiresAt = timer ? new Date(now.getTime() + timer * 60000) : new Date(now.getTime() + 60 * 60000);
    ...
    createRequest({
      ...
      userAvatar: user.avatar, title: title.trim(), category: category!,
      urgency, when, location: { name: user.city || 'Koramangala', distance: 0 },
      seatsTotal: seats[0], seatsTaken: 0, expiresAt, timer: timer ?? undefined, liveShare,
    });
    ...
    <div className="mobile-container min-h-screen bg-background">
      ...
      <div ref={contentRef} className="px-5 pb-32 space-y-6">
        ...
        <div className="space-y-2">
          <label className="text-sm font-semibold text-muted-foreground">What do you need?</label>
          <ModernInput
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {categories.map((cat) => (
              <button
                ...
                className={`flex items-center gap-2.5 py-3.5 px-4 rounded-2xl text-sm font-semibold transition-all tap-scale text-left ${
                  category === cat
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            {([
              { value: 'now' as Urgency, label: '⚡ Now' },
              { value: 'today' as Urgency, label: '☀️ Today' },
              { value: 'week' as Urgency, label: '📅 Week' },
            ]).map((u) => (
              <button
                key={u.value}
                onClick={() => setUrgency(u.value)}
                className={`flex-1 py-3.5 rounded-2xl text-sm font-semibold transition-all tap-scale ${
                  urgency === u.value
        ...
        {urgency === 'now' && (
          <div className="space-y-3">
            <label className="text-sm font-semibold text-muted-foreground">⏰ Auto-expire after</label>
            <div className="flex gap-2">
              {timers.map((t) => (
                <button
                  key={t.label}
                  ...
                  className={`flex-1 py-3 rounded-xl text-xs font-semibold transition-all tap-scale ${
                    timer === t.value
                      ? 'bg-secondary/15 text-secondary ring-2 ring-secondary/30'
                      : 'uber-card text-foreground'
                  }`}
                >
                  {t.label}
        ...
        {/* Location */}
        <div className="uber-card-elevated p-4 space-y-4">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <label className="text-sm font-semibold text-muted-foreground">👥 How many can join?</label>
          ...
          <div className="flex justify-between text-xs text-muted-foreground">
            <span>Solo buddy</span>
            <span>Full crew</span>
          </div>
        </div>
        
        {/* Credit Cost */}
        <div className="uber-card-elevated p-4 bg-accent/8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="text-2xl">💰</span>
              <div>
            ...
            <span ref={costRef} className="text-3xl font-extrabold">{creditCost}</span>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-5 glass-nav safe-area-pb">
        <div className="max-w-md mx-auto">
          <Button
            className="w-full h-14 text-base font-bold rounded-2xl bg-foreground text-background hover:opacity-90 disabled:opacity-40"
