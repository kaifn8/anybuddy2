...
const STATUS_CONFIG: Record<VerificationStatus, {
  ...
  unverified: {
    icon: <Camera size={18} />,
    title: 'Get verified',
    description: 'Upload a selfie to verify your identity. It helps others trust you.',
  ...
  pending: {
    ...
    description: 'Your selfie is under review. We\'ll notify you once it\'s approved.',
    color: 'text-warning',
  ...
  verified: {
    ...
    title: 'Verified',
    description: 'Verified means this person completed selfie verification.',
    color: 'text-primary',
  ...
  failed: {
    ...
    description: 'Your selfie didn\'t pass review. Please try again with a clear photo.',
...
export function VerificationCard() {
  ...
  return (
    <div className={cn(
      ...
      status === 'verified' ? 'border-primary/20 bg-primary/[0.04]' :
      status === 'pending' ? 'border-warning/20 bg-warning/[0.04]' :
      status === 'failed' ? 'border-destructive/20 bg-destructive/[0.04]' :
      'border-border/30 bg-background/60'
    )}>
      {/* Header */}
      <div className="flex items-center gap-2.5 mb-2">
        <div className={cn('w-8 h-8 rounded-xl flex items-center justify-center shrink-0', config.bgColor, config.color)}>
          {config.icon}
        </div>
        <div className="flex-1 min-w-0">
          <p className={cn('text-sm font-semibold', status === 'verified' ? 'text-primary' : 'text-foreground')}>
            {config.title}
          </p>
      ...
      {showUpload && preview && (
        <div className="mt-3 space-y-2.5">
          <div className="flex items-center gap-3 p-2.5 rounded-xl bg-muted/30">
            ...
            <p className="text-[11px] text-muted-foreground flex-1">Looking good! This will be used only for verification.</p>
          </div>
